...
'''
    ...
    node(label) {
        ...
        stage('Checkout SCM') {
            git credentialsId: 'git', url: 'https://github.com/Kumar-arj/microservice-demo-shipping.git', branch: 'master'
            container('build') {
                ...
                container('docker') {
                    stage('Build Image') {
                        docker.withRegistry( 'https://registry.hub.docker.com', 'dockerhub' ) {
                            def customImage = docker.build('kumararj/sock-shop-micro-services-shipping:latest')
                            customImage.push()
                        }
                    }
                }
                ...
                container('build') {
                    dir('charts') {
                        withCredentials([usernamePassword(credentialsId: 'nexuslogin', usernameVariable: 'username', passwordVariable: 'password')]) {
                            ...
                            // '''
                            sh '/usr/local/bin/helm package micro-services-shipping'
                            ...
                            sh 'curl -v -u $username:$password --upload-file micro-services-shipping-1.0.tgz http://nexus.k4m.in/repository/sock-shop-helm-local/'
